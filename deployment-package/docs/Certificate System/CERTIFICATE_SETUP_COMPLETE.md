...
### Step 1: Fix Certificate System (REQUIRED)
**IMPORTANT**: The certificate system is incomplete and needs to be fixed first.

Run the certificate system fix:
```bash
./scripts/fix-certificate-system.sh
```

This will:
- Generate missing PEM files (fullchain.pem, privkey.pem)
- Generate missing P12 files for Windows import
- Deploy certificates to server
- Configure Traefik with SSL

### Step 2: Install Landing Dashboard Certificate
...
### Step 3: Verify Certificate Works
...
### Step 4: Install Additional Service Certificates
