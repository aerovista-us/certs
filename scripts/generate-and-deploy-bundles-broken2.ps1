...
# Logging functions
function Write-Log($message) { Write-Host "🔐 $message" -ForegroundColor Blue }
function Write-Success($message) { Write-Host "✅ $message" -ForegroundColor Green }
function Write-Warning($message) { Write-Host "⚠️ $message" -ForegroundColor Yellow }
function Write-Error($message) { Write-Host "❌ $message" -ForegroundColor Red }

Write-Log "🔐 NXCore Generate and Deploy Bundles - Starting..."

# Create backup directory
New-Item -ItemType Directory -Path $BackupDir -Force | Out-Null

# Phase 1: Generate Full Key Bundles (Windows)
Write-Log "📦 Phase 1: Generating full key bundles on Windows..."
...
function Generate-FullBundle {
    ...
    Write-Log "Generating full certificate bundle for $ServiceName..."
    ...
    Write-Success "Full certificate bundle generated for $ServiceName"
}
...
Write-Success "All certificate bundles generated on Windows"

# Phase 2: Deploy to NXCore Server
Write-Log "🚀 Phase 2: Deploying certificates to NXCore server..."
...
Write-Log "Deploying main certificates to server..."
...
Write-Success "Main certificates deployed to server"

# Phase 3: Deploy All Service Certificates to Server
Write-Log "📦 Phase 3: Deploying all service certificates to server..."
...
foreach ($Service in $Services) {
    $ServiceDir = Join-Path $CertsDir $Service
    if (Test-Path $ServiceDir) {
        Write-Log "Deploying $Service certificates to server..."
...
"@
        ...
        Write-Success "$Service certificates deployed to server"
    }
}

# Phase 4: Configure Traefik with Certificates
Write-Log "🔧 Phase 4: Configuring Traefik with certificates..."
...
Write-Success "Traefik SSL configuration deployed"

# Phase 5: Restart Traefik
Write-Log "🔄 Phase 5: Restarting Traefik with new certificates..."
...
Write-Success "Traefik restarted with new certificates"

# Phase 6: Create Client Installation Packages
Write-Log "📦 Phase 6: Creating client installation packages..."
...
function Create-InstallationPackage {
    ...
    Write-Log "Creating installation package for $ServiceName..."
...
"@
    ...
    Write-Success "Installation package created for $ServiceName"
}
...
Write-Success "All client installation packages created"

# Phase 7: Test Certificate System
Write-Log "🧪 Phase 7: Testing certificate system..."

# Test certificate connectivity
Write-Log "Testing certificate connectivity..."
try {
    $Response = Invoke-WebRequest -Uri "https://$Domain/" -UseBasicParsing -SkipCertificateCheck
    if ($Response.StatusCode -eq 200) {
        Write-Success "Certificate system working (HTTP $($Response.StatusCode)) ✅"
    } else {
        Write-Warning "Certificate system issue (HTTP $($Response.StatusCode)) ⚠️"
    }
} catch {
    Write-Warning "Certificate system test failed: $($_.Exception.Message)"
}

# Phase 8: Generate Deployment Report
Write-Log "📋 Phase 8: Generating deployment report..."
...
Write-Success "Generate and deploy report generated"

# Final success message
Write-Success "🎉 NXCore Certificates Generated and Deployed!"
Write-Success "✅ Full key bundles generated on Windows"
Write-Success "✅ Certificates deployed to server"
Write-Success "✅ Traefik configured with SSL"
Write-Success "✅ Client installation packages ready"

Write-Log "📋 Deployment report saved to: $BackupDir\generate-and-deploy-report.md"
Write-Log "🔐 Certificate system is now complete and deployed"
