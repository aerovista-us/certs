...
rem ================================================================
rem AeroCaller: Startup Script (Windows)
...
set "LOG_DIR=%LOCALAPPDATA%\AeroVista\AeroCaller\logs"
...
echo [%date% %time%] Starting AeroCaller... >> "%LOG_FILE%"
...
if %errorlevel%==0 (
  ...
  start "AeroCaller" /min cmd /c "set USE_HTTPS=%USE_HTTPS%&& set PORT=%PORT%&& set KEY_PATH=%KEY_PATH%&& set CERT_PATH=%CERT_PATH%&& node server.staff.js >> ^"%LOG_FILE%^" 2>>&1"
)
...
rem ---- Wait for server ready and open browser ---------------------------------
set "PROTO=http"
if "%USE_HTTPS%"=="1" set "PROTO=https"

echo Waiting for %PROTO%://127.0.0.1:%PORT%/api/readyz ... >> "%LOG_FILE%"
powershell -NoProfile -Command "$ErrorActionPreference='SilentlyContinue'; for($i=0;$i -lt 40;$i++){ try{ $r=Invoke-WebRequest -UseBasicParsing '%PROTO%://127.0.0.1:%PORT%/api/readyz'; if($r.StatusCode -eq 200){ exit 0 } } catch{}; Start-Sleep -Milliseconds 250 }; exit 1"
if %errorlevel%==0 (
  start "" %PROTO%://localhost:%PORT%/index.staff.html
  echo Opened %PROTO%://localhost:%PORT%/index.staff.html >> "%LOG_FILE%"
) else (
  echo Server not ready after wait; you can open %PROTO%://localhost:%PORT%/index.staff.html manually. >> "%LOG_FILE%"
)
